    ...
    return (
        <div className='receipt-popup-overlay text-dark'>
            <div className="receipt-popup">
                ...
                <p>
                    <strong>Order Id :</strong>{orderDetails.OrderId || orderDetails.orderId}
                </p>
                <p>
                ...
                <div className="d-flex justify-content-between mb-2">
                    ...
                    <span>₹{orderDetails.grandTotal?.toFixed(2) || '0.00'}</span>
                </div>
                <p>
                    <strong>Payment Method :</strong>{orderDetails.paymentMethod}
                </p>
                {
                    (orderDetails.paymentMethod === "UPI" || orderDetails.paymentMethod === "upi") && (
                        <>
                            {orderDetails.stripeOrderId && (
                                <p>
                                    <strong>Stripe Order Id :</strong> {orderDetails.stripeOrderId}
                                </p>
                            )}
                            {(orderDetails.stripePaymentId || orderDetails.paymentDetails?.stripePaymentIntentId) && (
                                <p>
                                    <strong>Stripe Payment Id :</strong> {orderDetails.stripePaymentId || orderDetails.paymentDetails?.stripePaymentIntentId}
                                </p>
                            )}
                        </>
                    )
